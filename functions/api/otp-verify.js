...
// Checks the code against what /api/otp-start stored in KV. On success,
// issues a signed session token (30 days) binding email+name+phone, so
// /api/tutor can trust the identity without re-checking KV every message.
// Also logs the verified sign-up into the same Google Sheet the
// homepage trial-booking form writes to, so tutor leads land alongside
...
// so no new secret is needed to reuse it here.
...
export async function onRequestPost(context) {
  ...
  // Correct — consume the code and issue a session token.
  await env.TUTOR_KV.delete(kvKey);
  ...
  // Fire-and-forget lead log — never let a sheet hiccup block verification.
  context.waitUntil(logLeadToSheet({ email, name: record.name, phone: record.phone, grade: record.grade }));
